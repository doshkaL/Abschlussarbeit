...
def receive_user():
    try:
        ...
        salt = bcrypt.gensalt()
        hashed_password = bcrypt.hashpw(user_dto.password.encode('utf-8'), salt).decode('utf-8')

        # Map DTO to Model
        user = user_dto_to_model(user_dto)
        user.password = hashed_password  # Gehashtes Passwort setzen
...
def get_instructor_courses():
    try:
          # Get the instructor's username from the request parameters
        instructor_username = request.args.get('username')
        if not instructor_username:
            return jsonify({'error': 'Instructor username is required'}), 400
            ...
            for student in courses[course_id]["students"]:
                if student["username"] == row.student_username:
                    ...
                    student["exercises"].append({
                        ...
                        "due_date": row.exercise_due_date.isoformat() if row.exercise_due_date else None,
                        "submitted_at": row.exercise_submitted_at.isoformat() if row.exercise_submitted_at else None
                    })
                    break
...
def get_user():
    ...
    if not user:
        return jsonify({'error': 'User not found'}), 404  # Falls Benutzer nicht existiert

    #  Rückgabe mit gehashtem Passwort (Sicherheit beachten!)
    return jsonify({
        'username': user.username,
        'password_hash': user.password,  #Hier wird das gehashte Passwort zurückgegeben
        'role': user.role
    }), 200
